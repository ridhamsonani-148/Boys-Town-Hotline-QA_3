...
function validateTranscriptItem(item: any): TranscriptItem {
  ...
  const hasMaliciousContent = 
    /<script[^>]*>/gi.test(item.Content) ||
    /<iframe[^>]*>/gi.test(item.Content) ||
    /javascript:/gi.test(item.Content) ||
    /on\w+\s*=/gi.test(item.Content);

  if (hasMaliciousContent) {
    console.error('SECURITY ALERT: Malicious content detected in transcript', {
      itemId: item.Id,
      role: item.ParticipantRole,
      contentPreview: item.Content.slice(0, 100)
    });
    throw new Error('Malicious content detected in transcript - possible S3 tampering');
  }
  ...
  // If we reach here, all validation passed - return validated data
  return {
    ParticipantRole: sanitizedRole,
    Content: item.Content,  // Validated (not sanitized) - passed all checks above
...
function validateTranscribeOutput(data: any): TranscribeOutput {
  ...
  if (summaryContent) {
    const hasMaliciousSummary = 
      /<script[^>]*>/gi.test(summaryContent) ||
      /<iframe[^>]*>/gi.test(summaryContent) ||
      /javascript:/gi.test(summaryContent) ||
      /on\w+\s*=/gi.test(summaryContent);

    if (hasMaliciousSummary) {
      console.error('SECURITY ALERT: Malicious content detected in summary');
      throw new Error('Malicious content detected in summary - possible S3 tampering');
    }

    if (summaryContent.length > 5000) {
  ...
  return {
    ...
    ConversationCharacteristics: {
      ContactSummary: {
        AutoGenerated: {
          OverallSummary: {
            Content: summaryContent || ''  // Validated (not sanitized) - passed all checks above
          }
        }
      }
    }
