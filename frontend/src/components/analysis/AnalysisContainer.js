...
const ClipboardIcon = () => (
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
    <path
      d="M14 2H12.5C12.1 0.8 11 0 9.7 0H8.3C7 0 5.9 0.8 5.5 2H4C2.9 2 2 2.9 2 4V16C2 17.1 2.9 18 4 18H14C15.1 18 16 17.1 16 16V4C16 2.9 15.1 2 14 2ZM9 1.5C9.3 1.5 9.5 1.7 9.5 2S9.3 2.5 9 2.5S8.5 2.3 8.5 2S8.7 1.5 9 1.5ZM14 16H4V4H5.5V3.5C5.5 3.2 5.7 3 6 3H12C12.3 3 12.5 3.2 12.5 3.5V4H14V16Z"
      fill="#000"
    />
  </svg>
);
...
function AnalysisContainer({ fileName, onBackToUpload }) {
  ...
  useEffect(() => {
    ...
    uploadService.setStatusChangeCallback((status, results) => {
      ...
      // Support both new (originalFileName) and old (fileName) formats
      const targetFileName = results?.originalFileName || results?.fileName;
      if (!targetFileName) {
        console.warn(
          "Status change received without file identifier, ignoring"
        );
        return;
      }
          ...
          prevDetails.map((detail) => {
            ...
            if (isFileNameMatch(detail.name, targetFileName)) {
              return {
                ...detail,
                agentName:
                  results.agentName || results.agent || detail.agentName,
                status: "completed",
                score: `${
                  results.score || results.totalMultipliedScore || "92"
                } / 92`,
              };
            }
